...
class CamelResponse:
    ...
    def __init__(self, response: requests.Response, headers: dict, router_validation_key: str = None) -> None:
        self.response = response
        self.headers = headers
        self.validated_objects = None
    ...
    def validate(self, schema: BaseModel, validation_key: str = None) -> 'CamelResponse':
        if not validation_key:
            validation_key = self.router_validation_key
        try:
            self.validated_objects = Validator(schema, self.response_data, validation_key).fetch()
            return self
